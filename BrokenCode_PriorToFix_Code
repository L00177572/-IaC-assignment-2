...
TrustedHost.
 ...
 2. Windows Remote Management (HTTP-In) needs to be enables. use NewNetFirewallRule to set the firewall rules.
...
the process (Foreach).
 ...
 # Check if any security errors or warning was log to the
eventlog
...
function Get-UserDetail
{
 ...
 $userName = (Get-WmiObject -Class win32_computersystem -ComputerName
...
Security -Before $DateBefore -After $DateAfter | Where-Object {$_.EntryType -
like 'Error' -or $_.EntryType -like 'Warning'}
